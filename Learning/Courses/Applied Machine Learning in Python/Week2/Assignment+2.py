...
def answer_five():
    ...
    from adspy_shared_utilities import feature_importances_
    clf = DecisionTreeClassifier().fit(X_train2,y_train2,random_state = 0)
    importance_variables = pd.DataFrame({'feature': X_train2.columns.values, 'importance': clf.feature_importances_})
    return df.sort(['importance'], ascending=[False])['feature'].head(5).tolist()
